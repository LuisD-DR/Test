...
# app_streamlit.py
"""
App única que integra:
 - Carga del dataset Iris
 - EDA: histogramas, scatter matrix
 - Preprocesamiento (LabelEncoder, StandardScaler)
 - Entrenamiento RandomForest (botón para entrenar/actualizar)
 - Métricas (accuracy, precision, recall, f1, matriz confusión, reporte)
 - Guardado / carga de modelo con joblib
 - Dashboard estilo "Panel Universitario" (KPIs, pestañas)
 - Panel de predicción con scatter 3D que sitúa la nueva muestra
"""
...
st.caption("Proyecto integrado: EDA, entrenamiento y dashboard (único archivo).")
...
if df_raw is None:
    st.error("No se encontró Iris.csv en las rutas conocidas. Sube el archivo 'Iris.csv' en el mismo directorio o usa el uploader abajo.")
    uploaded_file = st.file_uploader("Sube Iris.csv (o arrastra aquí)", type=["csv"])
...
# seleccion de años ficticios no aplica; usamos filtros por especie si se desea
...
# ---------------------------
st.header("📊 Indicadores clave del dataset")
...
col2.metric("Especies únicas", f"{unique_species}")
...
# Pestañas
# ---------------------------
tab1, tab2, tab3 = st.tabs(["📈 Exploración", "🧠 Modelado", "🔮 Predicción"])
...
# --- TAB 2: Modelado ---
with tab2:
    st.subheader("Entrenamiento y métricas del modelo")
    ...
    if model_bundle:
        ...
        st.info("No se encontró un modelo guardado. Usa 'Entrenar / Actualizar modelo' para entrenar uno en este archivo.")

    st.markdown("---")

    # Entrenamiento (trigger por botón)
    if train_btn:
        st.info("Entrenando modelo... esto puede tardar unos segundos.")
        with st.spinner("Entrenando Random Forest y calculando métricas..."):
            try:
                X, y = prepare_Xy(df)
                ...
                fig_cm.update_layout(title="Matriz de confusión (test set)", xaxis_title="Predicción", yaxis_title="Verdadero")
                st.plotly_chart(fig_cm, use_container_width=True)

                st.markdown("**Reporte de clasificación (texto)**")
    ...
    st.markdown("**Entrenamiento local (explicación)**")
...
# --- TAB 3: Predicción ---
with tab3:
    st.subheader("Panel de predicción y visualización 3D")

    st.markdown("Introduce las medidas de la flor y pulsa 'Predecir' para ver la especie estimada y la posición de la muestra en un scatter 3D respecto al dataset.")
    ...
    if model_bundle is None:
        ...
        st.markdown("**Scatter 3D** — Sepal length (x), Petal length (y), Petal width (z)")
        ...
        # Añadir el punto nuevo si hay valores
        try:
            if pred_button:
...
# ---------------------------
# Footer: información y descargas
# ---------------------------
st.markdown("---")
st.caption("App integrada — Entrenamiento y dashboard en un solo archivo.")
...
with colf1:
    if os.path.exists(MODEL_PATH):
        ...
        with open(MODEL_PATH, "rb") as f:
            st.download_button("📥 Descargar modelo (.joblib)", data=f, file_name="model_iris_rf.joblib")
    else:
...
with colf2:
    ...
    if os.path.exists(PDF_PATH):
        with open(PDF_PATH, "rb") as f:
            st.download_button("📥 Descargar enunciado (PDF)", data=f, file_name="IRIS_SPECIES_CLASSIFICATION_PROYECT.pdf")
